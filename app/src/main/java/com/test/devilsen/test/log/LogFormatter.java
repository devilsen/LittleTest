...
class LogFormatter {
    ...
    static int METHOD_OFFSET = 0;
    static boolean SHOW_THREAD_INFO = false;
     ...
     * 调用栈偏移量
     *
     * @param methodOffset 这样可以拿到整个方法的调用栈，通过设置偏移量来打印具体的几个方法名
     */
    void setMethodOffset(int methodOffset) {
     ...
     * 显示调用线程名
     */
    void setShowThreadInfo() {
        showThreadInfo = true;
    }

    void log(int priority, @Nullable String onceOnlyTag, @NonNull String message) {
    ...
    private int getStackOffset(@NonNull StackTraceElement[] trace) {
        ...
        for (int i = MIN_STACK_OFFSET; i < trace.length; i++) {
            ...
            if (!name.equals(LogPrinter.class.getName()) && !name.equals(ELog.class.getName())) {
                return --i;
            }
        }
        return -1;
